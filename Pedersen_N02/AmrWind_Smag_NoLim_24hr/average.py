...
def calc_average_profile_last_hour():
  ...
  tstart_idx = np.searchsorted(a.variables['time'][:],82800.0)
...
def calc_average_istats_last_hour():
  ...
  i_stats = { v: float(np.average( a.variables[v][ a.variables['time'][:] > 82800.0] )) for v in vars}
...
def calc_average_spectra_last_hour():
  ...
  start_idx = np.searchsorted(tttime, 82800)
